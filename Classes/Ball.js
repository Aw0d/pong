class Ball {
    ...
    if (!this.boost) {
      this.speed =
        ...
        (this.maxSpeed - this.initialSpeed) *
          (1 - Math.exp((-this.acceleration / 100) * time));
    } else {
      this.speed =
        this.initialSpeed +
        (this.maxSpeed - this.initialSpeed) *
          (1 - Math.exp(-this.acceleration * time));
    }
  }
  ...
  reset(canvas) {
    ...
    this.y = canvas.height / 2;
    this.speedX =
      Math.sin(45 * (Math.PI / 180)) * (Math.random() > 0.5 ? 1 : -1);
    this.speedY =
      Math.sin(45 * (Math.PI / 180)) * (Math.random() > 0.5 ? 1 : -1);
  ...
  paddleCollisionDetection(paddle1, paddle2, canvas) {
    ...
    if (
      ...
      // Si la balle touche la paddle 1
      const ballPaddleDiff = round(
        this.y - this.radius - (paddle1.y + paddle1.height / 2)
      );
      const angle = round(
        90 -
          (ballPaddleDiff / (paddle1.height / 2 + this.radius)) *
            (90 - (180 - this.maxAngle))
      );
      ...
      // Si la balle touche la paddle 2
      const ballPaddleDiff = round(
        this.y - this.radius - (paddle2.y + paddle2.height / 2)
      );
      const angle = round(
        90 -
          (ballPaddleDiff / (paddle2.height / 2 + this.radius)) *
            (90 - (180 - this.maxAngle))
      );
